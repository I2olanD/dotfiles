...
description: Systematic test-driven development with proper test isolation and the RED-GREEN-REFACTOR cycle
license: MIT
...
type assert = ({ given: string, should: string, actual: any, expected: any }) {
  ...
  - Explicit - Everything you need to know to understand the test should be part of the test itself.
}
...
- UI testing strategy:
  ...
  - Side effects: must be isolated from UI, see `javascript-saga-effects` skill
